...
if (defined $number_to_guess and defined $guess) {
    ...
    if ($guess == $number_to_guess) {
        print "<p class=\"font\">You guessed right, it was $number_to_guess.<\p>\n";
        $game_over = 1;
    } elsif ($guess < $number_to_guess) {
        print "<p class=\"font\">Its higher than $guess.<\p>\n";
    } else {
        print "<p class=\"font\">Its lower than $guess.<\p>\n";
    }
} else {
    $number_to_guess = 1 + int(rand $max_number_to_guess);
    print "<p class=\"font\">I've  thought of number 0..$max_number_to_guess<\p>\n";
}

if ($game_over) {
print <<eof;
    <form method="POST" action="">
        <input type="submit" value="Play Again" class ="buttonstyle">
    </form>
eof
} else {
print <<eof;
    <form method="POST" action="">
        ...
        <input type="hidden" name="number_to_guess" value="$number_to_guess" class="button">
    </form>
eof
}
